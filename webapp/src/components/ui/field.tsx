...
function FieldSet({ className, ...props }: React.ComponentProps<"fieldset">) {
  return (
    <fieldset
      data-slot="field-set"
      className={cn(
        "flex flex-col gap-4 has-[>[data-slot=checkbox-group]]:gap-3 has-[>[data-slot=radio-group]]:gap-3 border px-4 py-6 bg-white rounded-lg",
        className,
      )}
      {...props}
    />
...
function FieldLegend({ className, variant = "legend", ...props }: React.ComponentProps<"legend"> & { variant?: "legend" | "label" }) {
  return (
    <legend
      ...
      className={cn("font-semibold uppercase data-[variant=label]:text-sm data-[variant=legend]:text-sm", className)}
      {...props}
    />
  );
}
